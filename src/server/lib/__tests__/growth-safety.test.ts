...
describe('growth post slots', () => {
    it('maps UTC hours to deterministic r/urjo growth slots', () => {
        ...
        expect(getGrowthPostSlot(new Date('2026-06-02T23:00:00Z'))).toBe('race_hour')
    })
    ...
    it('enables slots according to configured post frequency', () => {
        ...
        expect(isGrowthPostSlotEnabled('twice_daily', 'race_hour')).toBe(false)
        expect(isGrowthPostSlotEnabled('thrice_daily', 'race_hour')).toBe(true)
    })
})
...
testGrowthSlot('claimGrowthPostSlot allows one post per subreddit/date/slot', async () => {
    ...
    const otherSlot = await claimGrowthPostSlot('2026-06-02', 't5_urjo', 'race_hour')
