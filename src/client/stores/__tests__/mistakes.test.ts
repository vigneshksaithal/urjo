...
// Feature: unique-solution-validation, Property 2
// Validates: Requirements 2.1

import * as fc from 'fast-check'

// Known valid 4×4 base patterns — all constraints satisfied (balance, no adjacent
// identical rows/columns, no number constraints). Same patterns used in validation.test.ts.
const VALID_4x4_BASES: ReadonlyArray<ReadonlyArray<('red' | 'blue')[]>> = [
...
const makeValidGridFromBase = (base: ReadonlyArray<ReadonlyArray<'red' | 'blue'>>): Grid =>
    base.map((row) => row.map((color) => makeCell(color)))

describe('onCellChange — Property 2: Bug Condition - Valid Cells Not Flagged as Mistakes', () => {
    it('does not flag any cell in a valid grid as a mistake', () => {
        // **Validates: Requirements 2.1**
        // Property: for any valid grid and any cell position, touching that cell
        // and then moving to a different cell should NOT count a mistake.
        fc.assert(
            fc.property(
                // Pick a valid base pattern
                fc.integer({ min: 0, max: VALID_4x4_BASES.length - 1 }),
                // Pick a cell position (row, col) to "touch"
                fc.integer({ min: 0, max: 3 }),
                fc.integer({ min: 0, max: 3 }),
                // Pick a DIFFERENT cell to move to (to trigger evaluation of the first cell)
                fc.integer({ min: 0, max: 3 }),
                fc.integer({ min: 0, max: 3 }),
                (baseIdx, row, col, otherRow, otherCol) => {
                    const base = VALID_4x4_BASES[baseIdx]!
                    const grid = makeValidGridFromBase(base)
                    const gridSize = 4

                    // Reset before each property run to ensure isolation
                    resetMistakes()

                    const color = grid[row]![col]!.color as CellColor

                    // Touch the cell at (row, col) — sets lastActiveCell
                    onCellChange(row, col, color, grid, gridSize)

                    // Only trigger evaluation if we move to a DIFFERENT cell
                    if (otherRow !== row || otherCol !== col) {
                        const otherColor = grid[otherRow]![otherCol]!.color as CellColor
                        onCellChange(otherRow, otherCol, otherColor, grid, gridSize)
                    }

                    // No mistakes should have been counted — the grid is fully valid
                    expect(get(mistakeCount)).toBe(0)
                }
            ),
            { numRuns: 200 }
