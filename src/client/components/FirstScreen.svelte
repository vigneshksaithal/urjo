<script lang="ts">
    ...
    type Props = {
        ...
        /** A/B test variant — controls layout. Defaults to A (unchanged baseline). */
        variant?: "A" | "B";
        /** Player's current streak — used by Variant B for loss-aversion badge. */
    ...
    function buildPreviewCells(source: SerializedPuzzle): PreviewCell[] {
        return source.colors.split("").map((color, index) => {
            ...
            return {
                color: color === "r" ? "red" : color === "b" ? "blue" : null,
                number:
...
>
    {#if variant === "B"}
        <!-- ─── Variant B: social-proof hero + competitive CTA ─────────────── -->
        <!-- Hypothesis: moving social proof to hero and naming the target in   -->
        <!-- the button increases first-action rate vs. generic "Play".         -->
        <div class="flex flex-col items-center gap-1 text-center">
            ...
            {/if}
            {#if communityStats.activePlayers > 0}
                <p class="text-base font-bold text-theme-text-primary mt-1">
                    {communityStats.activePlayers.toLocaleString()} players solved
                    today
                </p>
            {/if}
        </div>
        ...
        <div class="flex flex-col items-center gap-3 w-full max-w-xs">
            ...
            <button
                onclick={onPlay}
                class="w-full px-10 py-3 bg-theme-text-primary text-theme-bg-primary font-bold rounded-lg text-base hover:opacity-90 active:scale-95 transition-all shadow-lg"
            >
                {targetLabel}
            </button>
        </div>
    {:else}
        <!-- ─── Variant A: existing layout unchanged (baseline) ─────────────── -->
        <div class="flex flex-col items-center gap-1 text-center">
            <h1 class="text-3xl font-bold text-theme-text-primary">Urjo</h1>
            {#if puzzleNumber > 0}
                <p class="text-sm text-theme-text-muted">
        ...
        {#if communityStats.activePlayers > 0 || communityStats.collectiveStreakDays > 0}
            <p class="text-xs text-theme-text-muted text-center">
                ...
                · 🔥
                {communityStats.collectiveStreakDays.toLocaleString()} collective
                streak days
            </p>
        {/if}
    {/if}
