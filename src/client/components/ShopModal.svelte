<script lang="ts">
	...
	async function equipTitle(titleId: string) {
		...
		try {
			...
			error = err instanceof Error ? err.message : 'Failed to equip title'
		} finally {
			equippingTitle = null
		}
	}
		...
		>
			...
			<div class="flex items-center justify-between p-4 border-b border-theme-border">
				<div class="flex items-center gap-2">
					<h2 class="text-lg font-bold text-theme-text-primary">🏪 Title Shop</h2>
				</div>
				<button
					onclick={onClose}
			...
			<!-- Coin balance -->
			<div class="flex items-center justify-center gap-2 py-3 bg-theme-bg-secondary border-b border-theme-border">
			...
			<!-- Error message -->
			{#if error}
				<div class="px-4 py-2 bg-red-500/20 text-red-400 text-sm text-center">
					{error}
				</div>
			{/if}
						...
						{#each items as item}
							...
							>
								<div class="flex items-center gap-3">
									...
									<div>
										<p class="font-semibold text-theme-text-primary text-sm">{item.label}</p>
										{#if item.condition && !item.unlocked}
											<p class="text-xs text-red-400">🔒 {getConditionText(item.condition)}</p>
										{:else if !item.owned && item.cost > 0}
											<p class="text-xs text-yellow-400">🪙 {item.cost}</p>
										{:else if !item.owned}
								...
								{#if item.equipped}
									<span class="text-xs text-green-400 font-semibold px-3 py-1">✓ Equipped</span>
								{:else if item.owned}
									<button
									...
									>
										{equippingTitle === item.id ? '...' : 'Equip'}
									</button>
								{:else if item.unlocked}
									<button
										onclick={() => buyTitle(item.id)}
										disabled={buyingTitle === item.id || coins < item.cost}
										class="px-3 py-1 rounded bg-yellow-600 text-white text-xs hover:bg-yellow-500 disabled:opacity-40 transition-colors"
									>
										{buyingTitle === item.id ? '...' : 'Buy'}
									</button>
								{:else}
									<span class="text-xs text-gray-500 px-3 py-1">🔒 Locked</span>
								{/if}
							</div>
						{/each}
					</div>
